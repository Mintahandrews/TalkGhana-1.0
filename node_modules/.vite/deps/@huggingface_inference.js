...
function preprocess(template, options = {}) {
  ...
  return template.replace(/-%}\s*/g, "%}").replace(/\s*{%-/g, "{%").replace(/-}}\s*/g, "}}").replace(/\s*{{-/g, "{{").replace(/-#}\s*/g, "#}").replace(/\s*{#-/g, "{#").replace(/{%\s*generation\s*%}.+?{%\s*endgeneration\s*%}/gs, "");
}
...
function parse(tokens) {
  ...
  function expectIdentifier(name2) {
    if (!isIdentifier(name2)) {
      throw new SyntaxError(`Expected ${name2}`);
    }
    ++current;
  }
  function parseAny() {
  ...
  function isIdentifier(...names) {
    return current + names.length <= tokens.length && names.every((name2, i) => tokens[current + i].type === "Identifier" && name2 === tokens[current + i].value);
  }
  function parseText() {
  ...
  function parseJinjaStatement() {
    ...
    const name2 = tokens[current].value;
    let result;
    switch (name2) {
      case "set":
        ++current;
      ...
      default:
        throw new SyntaxError(`Unknown statement type: ${name2}`);
    }
    return result;
  }
...
function getData(type, partialTaskData = data_default17) {
  return {
    ...partialTaskData,
    id: type,
...
var TASKS_DATA = {
  ...
  "audio-text-to-text": getData("audio-text-to-text", data_default17),
  "automatic-speech-recognition": getData("automatic-speech-recognition", data_default4),
  "depth-estimation": getData("depth-estimation", data_default16),
  "document-question-answering": getData("document-question-answering", data_default5),
  "visual-document-retrieval": getData("visual-document-retrieval", data_default35),
  ...
  "keypoint-detection": getData("keypoint-detection", data_default42),
  "mask-generation": getData("mask-generation", data_default14),
  "multiple-choice": void 0,
  "object-detection": getData("object-detection", data_default15),
  "video-classification": getData("video-classification", data_default34),
  other: void 0,
  "question-answering": getData("question-answering", data_default19),
  "reinforcement-learning": getData("reinforcement-learning", data_default18),
  robotics: void 0,
  "sentence-similarity": getData("sentence-similarity", data_default20),
  summarization: getData("summarization", data_default21),
  "table-question-answering": getData("table-question-answering", data_default22),
  "table-to-text": void 0,
  "tabular-classification": getData("tabular-classification", data_default23),
  "tabular-regression": getData("tabular-regression", data_default24),
  "tabular-to-text": void 0,
  "text-classification": getData("text-classification", data_default29),
  "text-generation": getData("text-generation", data_default30),
  "text-ranking": getData("text-ranking", data_default31),
  "text-retrieval": void 0,
  "text-to-image": getData("text-to-image", data_default25),
  "text-to-speech": getData("text-to-speech", data_default26),
  "text-to-audio": void 0,
  "text-to-video": getData("text-to-video", data_default32),
  "text2text-generation": void 0,
  "time-series-forecasting": void 0,
  "token-classification": getData("token-classification", data_default27),
  translation: getData("translation", data_default28),
  "unconditional-image-generation": getData("unconditional-image-generation", data_default33),
  "video-text-to-text": getData("video-text-to-text", data_default43),
  "visual-question-answering": getData("visual-question-answering", data_default36),
  "voice-activity-detection": void 0,
  "zero-shot-classification": getData("zero-shot-classification", data_default37),
  "zero-shot-image-classification": getData("zero-shot-image-classification", data_default38),
  "zero-shot-object-detection": getData("zero-shot-object-detection", data_default39),
  "text-to-3d": getData("text-to-3d", data_default41),
  "image-to-3d": getData("image-to-3d", data_default40)
};
...
async function fetchInferenceProviderMappingForModel(modelId, accessToken, options) {
  ...
  if (inferenceProviderMappingCache.has(modelId)) {
    ...
    const resp = await ((options == null ? void 0 : options.fetch) ?? fetch)(
...
function getInferenceSnippets(model, accessToken, provider, inferenceProviderMapping, opts) {
  var _a;
  return model.pipeline_tag && model.pipeline_tag in snippets ? ((_a = snippets[model.pipeline_tag]) == null ? void 0 : _a.call(snippets, model, accessToken, provider, inferenceProviderMapping, opts)) ?? [] : [];
}
